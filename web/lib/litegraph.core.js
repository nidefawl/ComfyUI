		...
		*/
        ...
        function refreshHelper() {
          ...
          if (that.onSearchBox) {
            ...
            function str_match_test(haystack, needle) {
              var aNeedle = needle.split(" ");
              var nNeedle = aNeedle.length;
              var nFound = 0;
              for (var i = 0; i < nNeedle; i++) {
                if (haystack.indexOf(aNeedle[i]) !== -1) {
                  nFound++;
                }
              }
              return nFound == nNeedle;
            }
            function inner_test_filter(type, optsIn) {
              ...
              if (filter && ctor.filter != filter)
                return false;
              if ((!options.show_all_if_empty || str) && !str_match_test(type.toLowerCase(), str))
                return false;
